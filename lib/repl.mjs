...
export async function runServer({ command, args }) {
  ...
  const shutdown = () => {
    ...
    shuttingDown = closeSession.catch(() => {}).finally(() => native.close());
    return shuttingDown;
  };
  const enqueue = (line) => {
    if (!line) return;
  ...
  try {
    ...
    session = new WorkerJavaScriptSession({ native, limits: options });
  ...
  async function handle(line) {
    ...
    try {
      ...
      if (method === "tools/list") return send({ jsonrpc: "2.0", id, result: { tools: toolDefinitions(options.timeoutMs) } });
...
export function toolDefinitions(defaultTimeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  return [
    { name: "js", description: jsDescription(timeoutMs), annotations, inputSchema: { type: "object", additionalProperties: false, properties: { code: { type: "string", minLength: 1, maxLength: MAX_CODE_CHARS, description: "JavaScript source to execute with top-level await and the initialized cua API." }, timeout_ms: { type: "integer", minimum: 1, maximum: MAX_TIMEOUT_MS, description: `Optional timeout in milliseconds. Defaults to ${timeoutMs} and is capped at 300000.` }, title: { type: "string", minLength: 1, maxLength: 80, description: "Short user-facing description of what this code is doing." } }, required: ["code"] } },
