...
const MAX_APP_BINDINGS = 128;
const MUTATING_APP_METHODS = new Set([
  "click",
  "drag",
  "pressKey",
  "scroll",
...
function normalizedAppAccess(value) {
  const policy = value && typeof value === "object" ? value : {};
  const defaultAccess = policy.default === "deny" ? "deny" : "allow";
  const normalizeRules = (rules) => Array.isArray(rules)
    ? rules.filter((rule) => typeof rule === "string" && rule.trim()).map((rule) => rule.trim().toLowerCase())
    : [];
  return {
    default: defaultAccess,
    allow: new Set(normalizeRules(policy.allow)),
    deny: new Set(normalizeRules(policy.deny)),
  };
}

function appMatches(app, rules) {
  if (!app || typeof app !== "object") return false;
  return [app.id, app.bundleId, app.name, app.displayName]
    .filter((value) => typeof value === "string" && value.trim())
    .some((value) => rules.has(value.trim().toLowerCase()));
}

function appNameMatches(app, name) {
  if (!app || typeof app !== "object") return false;
...
export function createSandboxBridge(
  ...
  schedule,
  cancel,
  { safetyMode = "full", appAccess = {} } = {},
) {
  const bindings = new Map();
  const accessPolicy = normalizedAppAccess(appAccess);
  let nextBinding = 1;
  const isReadOnly = safetyMode === "read-only";
  const appIsAllowed = (app) => {
    if (appMatches(app, accessPolicy.deny)) return false;
    if (accessPolicy.allow.size > 0) return appMatches(app, accessPolicy.allow);
    return accessPolicy.default === "allow";
  };
  const assertNamedAppAllowed = async (name) => {
    if (accessPolicy.allow.size === 0 && accessPolicy.deny.size === 0) return;
    const apps = await api.listApps({ emit: false });
    const matches = Array.isArray(apps) ? apps.filter((app) => appNameMatches(app, name)) : [];
    if (matches.length > 0) {
      if (matches.some((app) => !appIsAllowed(app))) {
        throw new Error(`app access denied by policy: ${name}`);
      }
      return;
    }
    if (!appIsAllowed({ id: name, displayName: name })) {
      throw new Error(`app access denied by policy: ${name}`);
    }
  };
  const filterApps = (apps) => Array.isArray(apps) ? apps.filter(appIsAllowed) : [];
  const dispatch = async (method, args) => {
    try {
      ...
      if (method === "getState") {
        ...
        return encodeSandboxResult({ apps: filterApps(state.apps) });
      }
      if (method === "listApps") return encodeSandboxResult(filterApps(await api.listApps({ emit: false })));
      if (method === "getApp") {
        const app = args[0];
        if (typeof app !== "string" || !app.trim()) {
          throw new Error("cua.getApp expects a non-empty app name or bundle ID");
        }
        await assertNamedAppAllowed(app);
        if (bindings.size >= MAX_APP_BINDINGS) {
          throw new Error(`CUA app binding limit reached (${MAX_APP_BINDINGS}); call js_reset`);
        }
      ...
      if (method.startsWith("app:")) {
        const methodName = method.slice(4);
        if (isReadOnly && MUTATING_APP_METHODS.has(methodName)) {
          throw new Error(`mutating CUA method disabled in read-only safety mode: ${methodName}`);
        }
        const [token, ...callArgs] = args;
        const binding = bindings.get(token);
        if (!binding) throw new Error("unknown CUA app binding");
        const result = await binding[methodName](...callArgs);
